...
---
This page is the site's test suite, written down. What the build can check, it checks before every commit and fails loudly rather than quietly; what only an outside service can measure carries the date it was measured and a link to measure it again. If a row says fail, it is failing right now and the fix is queued, not hidden.
...
**How the writing is made.** Every piece here starts as a draft written by an AI model from my drafts and my own notes and working history; I edit what comes back and decide what ships. Before the first piece is published, a step goes in between: an agent with repository access that checks each technical claim against the code, the commits and the logs and marks it confirmed, wrong or unverifiable. It's the same shape as any review I'd defend at work — the machine does the volume, a second machine checks the facts, a human signs. Anything wrong that survives is mine.
