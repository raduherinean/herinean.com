...
SQL="SELECT $COL AS $BY, SUM(_sample_interval) AS views FROM herinean_views WHERE timestamp > NOW() - INTERVAL '$DAYS' DAY GROUP BY $BY ORDER BY views DESC"
curl -sS --fail-with-body -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" --data "$SQL" "https://api.cloudflare.com/client/v4/accounts/$CLOUDFLARE_ACCOUNT_ID/analytics_engine/sql" | jq -r --arg by "$BY" '(["\($by)","views"] | @tsv), (.data[] | [.[$by], (.views|tostring)] | @tsv)'
